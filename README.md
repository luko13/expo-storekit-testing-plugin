...
Generating iOS projects and running StoreKit Testing require macOS and Xcode. The plugin itself is evaluated during Expo configuration and prebuild.
...
Releases follow Semantic Versioning. Version `0.1.0` must first be published manually by the maintainer so that the npm package exists. After that bootstrap release, the package's npm Trusted Publisher is configured for the repository workflow and the `npm` environment; subsequent releases use OIDC from an explicitly dispatched stable SemVer tag. Prerelease tags are rejected so they cannot be promoted to npm's `latest` channel accidentally. No persistent npm publish token is required.
